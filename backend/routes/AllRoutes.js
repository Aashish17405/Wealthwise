...
const groq = new Groq({
  apiKey: "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",
});
...
async function getRecommendationFromGroq(userInput, recommendations) {
  ...
  const prompt = `
    ...
    Give me reasons behind your suggestion. Give me two stars in between the recommended mutual fund.
...
allroutes.post("/fdrecommendations", async (req, res) => {
  ...
  try {
    ...
    const prompt = `
      ...
      Please explain why this is the best choice in 500 to 600 characters. Format your response like this:
      **${bestRecommendation.bank}** is offering a maturity amount of **INR ${bestRecommendation.maturityAmount}** at an interest rate of **${bestRecommendation.interestRate}%**. 
      Then continue with your explanation of why this is the best choice.`;
...
allroutes.get("/findmail", async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ message: "User found", count: newUser.count });
  } catch (e) {
    console.error(e);
    return res.status(400).json({ error: e.message });
  }
...
allroutes.get("/stock/search", async (req, res) => {
  try {
...
// Route to buy stocks
allroutes.post("/stock/buy", async (req, res) => {
  try {
...
allroutes.post("/stock/sell", async (req, res) => {
  try {
    const { email, symbol, quantity } = req.body;
...
allroutes.get("/portfolio", async (req, res) => {
  try {
    const { email } = req.query;
